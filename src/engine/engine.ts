...
export class CreationEngine extends Application {
  ...
  public async init(opts: Partial<ApplicationOptions>): Promise<void> {
    ...
    // `manifest` is `any` here: src/manifest.json is generated by AssetPack, so on a
    // clean checkout it doesn't exist yet when tsc runs (hence the @ts-ignore above).
    // Asserting the real Pixi type restores inference and keeps `tsc` honest.
    const allBundles = (manifest as AssetsManifest).bundles.map(
      (item) => item.name,
    );
